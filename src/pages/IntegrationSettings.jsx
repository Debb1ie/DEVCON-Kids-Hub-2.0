...
import { FolderSync, Link2, RefreshCcw, Save, Sheet, Unlink } from 'lucide-react';
...
export default function IntegrationSettings() {
  ...
  return (
    <div className="integration-page">
      <PageHeader
        eyebrow="Server-side integration"
        title="Google Workspace"
        description="Connect the authorized DEVCON Google account and manage My Drive report automation."
      />
      ...
      <section className="card integration-connection">
        <div>
          <h2>Google authorization</h2>
          {form.connectedAt ? (
            <p>Connected as <strong>{form.connectedEmail}</strong>. Tokens remain encrypted and server-side.</p>
          ) : (
            <p>No Google account is connected. A Super Admin must authorize the dedicated DEVCON account.</p>
          )}
        </div>
        {form.connectedAt ? (
          <button type="button" className="btn-secondary" disabled={busy} onClick={disconnect}>
            <Unlink size={16} /> Disconnect and revoke access
          </button>
        ) : (
      ...
      <form className="card integration-form" onSubmit={save}>
        <div className="section-head">
          <div>
            ...
            <p>Only non-secret resource references are stored here. Credentials stay in Supabase secrets.</p>
          </div>
        </div>
        <label>
          <span><FolderSync size={17} /> Google Drive root folder URL or ID</span>
          <input
            value={form.googleDriveRoot}
            onChange={(e) => change('googleDriveRoot', e.target.value)}
            placeholder="My Drive folder URL or ID"
            autoComplete="off"
          />
        </label>
        <label>
          <span><Sheet size={17} /> Google Sheet URL or ID</span>
          <input
            value={form.reportSheet}
            onChange={(e) => change('reportSheet', e.target.value)}
            placeholder="Google Sheet URL or ID"
            autoComplete="off"
          />
        </label>
        <label>
          <span>Report sheet tab name</span>
          <input
            value={form.sheetTabName}
            onChange={(e) => change('sheetTabName', e.target.value)}
            maxLength={100}
          />
        </label>
        <label className="integration-toggle">
          <span>
            <strong>Automatic event folders</strong>
            <small>Create one deterministic folder when the first valid report is submitted.</small>
          </span>
          <input
            type="checkbox"
            disabled={!form.connectedAt}
            checked={form.automaticFolders}
            onChange={(e) => change('automaticFolders', e.target.checked)}
          />
        </label>
        <label className="integration-toggle">
          <span>
            <strong>Post Event Report Sheet sync</strong>
            <small>Queue updates after submission, revision requests, and approval.</small>
          </span>
          <input
            type="checkbox"
            disabled={!form.connectedAt}
            checked={form.sheetSync}
            onChange={(e) => change('sheetSync', e.target.checked)}
          />
        </label>
        <button className="btn-primary" disabled={busy} type="submit">
          <Save size={17} /> {busy ? 'Saving…' : 'Save integration settings'}
        </button>
      </form>
      ...
      <section className="card integration-jobs">
        <div className="section-head">
          <div>
            <h2>Job history</h2>
            <p>Failures never invalidate the event or report stored in Supabase.</p>
          </div>
          <button className="btn-secondary" type="button" disabled={busy} onClick={process}>
            <RefreshCcw size={16} /> Process pending jobs
          </button>
        </div>
        {jobs.length === 0 ? (
          <EmptyState
            icon={FolderSync}
            title="No integration jobs yet"
            description="Automation jobs will appear here when event folders or report sync runs."
          />
        ) : (
          <div className="integration-table-wrap">
            <table>
              ...
              <tbody>
                {jobs.map((job) => (
                  <tr key={job.id}>
                    <td>{job.job_type === 'create_event_folder' ? 'Event folder' : 'Report Sheet sync'}</td>
                    <td>
                      <span className={`integration-status ${job.status}`}>{job.status}</span>
                      {job.safe_error_message && <small>{job.safe_error_message}</small>}
                    </td>
                    <td>{job.attempt_count}</td>
                    <td>{new Date(job.updated_at).toLocaleString()}</td>
                    <td>
                      {job.status === 'failed' ? (
                        <button type="button" disabled={busy} onClick={() => retry(job.id)}>
                          Retry
                        </button>
                      ) : (
                        '—'
