...
export default function Inventory() {
  ...
  const canManageItem = (item) =>
    canPerform(roleKey, 'inventory.manage', {
  ...
  const handleAdd = async (e) => {
    ...
    if (!name.trim()) {
      setFormError('Enter an inventory item name.');
      return;
    }
    if (String(stock).trim() === '' || !Number.isInteger(stockValue) || stockValue < 0) {
      setFormError('Stock must be a whole number of 0 or more.');
      return;
    }
    if (imageUrl.trim() && !isValidHttpUrl(imageUrl.trim())) {
      setFormError('Enter a valid image URL beginning with http:// or https://.');
      return;
    }
    setFormError('');
    setFormSuccess('');
    ...
    const payload = {
      ...
      ...(user?.chapterId ? { chapter_id: user.chapterId } : {}),
    };

    try {
      if (editingId) {
        await updateInventoryItem(editingId, payload);
        setFormSuccess('Inventory item updated successfully.');
      } else {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setDeleteSuccess('Inventory item deleted successfully.');
    } catch (error) {
  ...
  const paginatedItems = filtered.slice(
    (activePage - 1) * INVENTORY_PER_PAGE,
    activePage * INVENTORY_PER_PAGE
  );
  ...
  return (
    <div className="module-page inventory-page">
      <PageHeader
        ...
        description="Track and allocate hardware kits, microcontrollers, and workshop materials."
        scope={scopeLabel}
        actions={
          canCreateItem && (
      ...
      {formSuccess && (
        <div className="inventory-form-feedback success inline-alert alert-success" role="status">
          {formSuccess}
        </div>
      )}
      {deleteSuccess && (
        ...
        (editingId
          ...
          <>
            ...
            <div className="inventory-modal-container">
              <div
                className="card animate-fade-in inventory-form-card"
                role="dialog"
                aria-modal="true"
                    ...
                    <div className="inventory-form-fields">
                      <div className="inventory-form-group">
                        ...
                        <input
                          ...
                          aria-invalid={formError === 'Enter an inventory item name.'}
                          required
                        />
                      </div>
                      <div className="inventory-form-group">
      ...
      <section className="inventory-stats" aria-label="Inventory metrics">
        <article className="card stat-card">
          <h4>Total Units</h4>
          <h2>
            {inventoryList?.reduce((acc, curr) => acc + (curr.stock || 0), 0) || 0}
          </h2>
        </article>
        <article className="card stat-card warning">
          <h4>Low Stock Types</h4>
          <h2>
            {inventoryList?.filter((item) => item.status === 'Low Stock').length || 0}
          </h2>
        </article>
        <article className="card stat-card danger">
          <h4>Out of Stock Types</h4>
          <h2>
            {inventoryList?.filter((item) => item.status === 'Out of Stock').length || 0}
          </h2>
        </article>
      </section>
      ...
      <div className="card list-container">
        <div className="list-toolbar">
          <div className="inventory-search-group">
            <label className="inventory-sr-only" htmlFor="inventory-search">
              Search inventory
            </label>
          ...
          <p className="inventory-result-count">
            {searchTerm
              ? `${filtered.length} of ${inventoryList?.length || 0} items`
              : `${inventoryList?.length || 0} inventory items`}
          </p>
        </div>
          ...
          <table className="data-table">
            <thead>
              <tr>
                ...
                {(roleKey === 'super_admin' ||
                  roleKey === 'admin' ||
                  roleKey === 'chapter_coordinator') && <th>Action</th>}
              </tr>
            </thead>
            <tbody>
              {paginatedItems.map((item) => (
                <tr key={item.id}>
                  ...
                  <td>
                    <div
                      className={`inventory-status ${(item.status || 'in-stock')
                        .replace(/\s+/g, '-')
                        .toLowerCase()}`}
                    >
                ...
                <tr>
                  ...
                  >
                    <EmptyState
                      ...
                      description={
                        (inventoryList || []).length === 0
                          ? 'Add hardware kits and materials to track workshop equipment across chapters.'
